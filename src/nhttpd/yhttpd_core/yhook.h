...
class Cyhook
{
	...
	public:
		...
		{
			return std::string("Abstract Hook Class");
		}
		// CWebserverConnection based hooks
		virtual THandleStatus 	Hook_PrepareResponse(CyhookHandler *)
		{
			return HANDLED_NONE;
		};
		virtual THandleStatus 	Hook_SendResponse(CyhookHandler *)
		{
			return HANDLED_NONE;
		};
		virtual THandleStatus	Hook_EndConnection(CyhookHandler *)
		{
			return HANDLED_NONE;
		}
		virtual THandleStatus	Hook_UploadSetFilename(CyhookHandler *, std::string &)
		{
			return HANDLED_NONE;
		}
		virtual THandleStatus	Hook_UploadReady(CyhookHandler *, std::string)
		{
			return HANDLED_NONE;
		}
		// Cyhttpd based hooks
		virtual THandleStatus 	Hook_ReadConfig(CConfigFile *, CStringList &)
		{
			return HANDLED_NONE;
		};
};
...
class CyhookHandler
{
	...
	public:
		...
		void SendFile(const std::string &url)
		{
			NewURL = url;
			status = HANDLED_SENDFILE;
		}
		void SendRedirect(const std::string &url)
		{
			httpStatus = HTTP_MOVED_TEMPORARILY;
			NewURL = url;
			status = HANDLED_REDIRECTION;
		}
		void SendRewrite(const std::string &url)
		{
			NewURL = url;
			status = HANDLED_REWRITE;
		}
