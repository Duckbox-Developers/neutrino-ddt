...
int CLuaInstMenu::MenuAddItem(lua_State *L)
{
	...
	{
		std::string right_icon_str;
		tableLookup(L, "right_icon", right_icon_str);
		std::string action;
		tableLookup(L, "action", action);
		std::string value;
		tableLookup(L, "value", value);
		std::string hint;
		tableLookup(L, "hint", hint);
		std::string hint_icon_str;
		tableLookup(L, "hint_icon", hint_icon_str);
		std::string icon_str;
		tableLookup(L, "icon", icon_str);
		std::string id;
		tableLookup(L, "id", id);
		std::string tmp;
		...
		else if (type == "keyboardinput")
		{
			...
			tableLookup(L, "size", size);
			std::string help = "";
			tableLookup(L, "help", help);
			std::string help2 = "";
			tableLookup(L, "help2", help2);
...
int CLuaInstMenu::MenuSetActive(lua_State *L)
{
	...
	lua_Integer id;
	tableLookup(L, "item", id);
	bool activ;
	tableLookup(L, "activ", activ);
...
int CLuaInstMenu::MenuSetSelected(lua_State *L)
{
	...
	lua_Integer preselected;
	tableLookup(L, "preselected", preselected);
