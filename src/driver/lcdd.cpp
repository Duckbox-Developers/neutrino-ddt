...
#if BOXMODEL_DM800 || BOXMODEL_DM800SE || BOXMODEL_DM800SEV2 || BOXMODEL_DM820 || BOXMODEL_DM7080	// lcd to small
	showmode = 0x01;
#endif
...
void CLCD::setScrollMode(int scroll_repeats)
{
	...
	if (scroll_repeats)
	{
		...
		proc_put("/proc/stb/lcd/scroll_delay", "333");
...
void CLCD::UpdateIcons()
{
	...
	if (chan)
	{
		...
		if (chan->getAudioChannel() != NULL)
		{
			...
			ShowIcon(FP_ICON_MP3, chan->getAudioChannel()->audioChannelType == CZapitAudioChannel::MPEG);
		}
	}
#endif
}
